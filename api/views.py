...
def create_image(name, properties, file):
    ...
    image = Image(name=name, width=width, height=height, channels=channels, file=file)
    image.save()
    return image
...
def create_pdf(name, properties, file):
    ...
    pdf = Pdf(name=name, width=width, height=height, page_count=page_count, file=file)
    pdf.save()
    return pdf
...
class ImageRetreiveView(RetrieveAPIView):
    queryset = Image.objects.all()
...
class PdfRetreiveView(RetrieveAPIView):
    queryset = Pdf.objects.all()
